# 🤖 Guía para Claude Design — Índice

Esta es la **guía principal** para usar este sistema de diseño con Claude Design.
La documentación está dividida en archivos por componente — usá este índice para encontrar cada uno.

> Mantenida por la skill `design-system` (`.claude/skills/design-system/SKILL.md`).

## 🎯 Componentes UI (`src/components/ui/`)

- [Avatar](Avatar.md)
- [Badge](Badge.md)
- [Breadcrumb](Breadcrumb.md)
- [Button](Button.md)
- [ButtonGroup](ButtonGroup.md)
- [Card](Card.md)
- [Carousel](Carousel.md)
- [Checkbox](Checkbox.md)
- [Combobox](Combobox.md)
- [Dialog](Dialog.md)
- [Field](Field.md)
- [IconTileCard](IconTileCard.md)
- [Input](Input.md)
- [InputGroup](InputGroup.md)
- [InputPhone](InputPhone.md)
- [Label](Label.md)
- [Select](Select.md)
- [Separator](Separator.md)
- [Switch](Switch.md)
- [Table](Table.md)
- [Text](Text.md)
- [Textarea](Textarea.md)
- [Tooltip](Tooltip.md)

## 🧩 Componentes Compuestos (`src/components/common/`)

- [ActionTableIcon](ActionTableIcon.md)
- [DataTable](DataTable.md)
- [EmployeeNameInfo](EmployeeNameInfo.md)
- [EmployeeStatusCard](EmployeeStatusCard.md)
- [IconsChartCard](IconsChartCard.md)
- [PieChart / PieChartApex](PieChart.md)
- [RingsChart / RingsChartApex](RingsChart.md)
- [StatusTile](StatusTile.md)
- [ToolbarButton](ToolbarButton.md)

## 🌟 Iconos custom (`src/components/icons/`)

- [Icons (IconWrapper + SVGs)](Icons.md)

## 📐 Layouts (`src/layouts/`)

- [AppLayout](AppLayout.md)

## 🎨 Tokens y patrones

- [Tokens de Diseño](Tokens.md) — variables CSS de `src/App.css`
- [Patterns](Patterns.md) — layout (container, grid, flex) y patrones comunes (login, navbar, hero, dashboard cards)
